...
def _ingest(config: Config, handler: Callable[[httpx.Request], httpx.Response]) -> Any:
    return document_collector.ingest_documents(config, transport=httpx.MockTransport(handler))
...
def test_rerunning_skips_unchanged_bytes_and_does_not_duplicate(tmp_config: Config) -> None:
    _seed_filing(tmp_config)
    first = _ingest(tmp_config, _handler())
    second = _ingest(tmp_config, _handler())
